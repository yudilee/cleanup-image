...
import { Upload, Eraser, Download, RefreshCw, Undo, RotateCcw, Paintbrush, SplitSquareHorizontal, X, Square, Lasso, Cpu, Zap, Wand2, Sparkles, ImageMinus, Expand, Layers, Share2, Clock } from "lucide-react";
...
export default function Home() {
  ...
  const handleClean = async () => {
    ...
    try {
      ...
      alert("Error processing image. Ensure backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4 mb-6 bg-neutral-800 p-4 rounded-xl shadow-lg border border-neutral-700">
        ...
        {/* AI Features */}
        {imageSrc && (
          <div className="flex items-center gap-1 bg-gradient-to-r from-purple-900/50 to-blue-900/50 rounded-lg p-1 border border-purple-500/30">
            <button
            ...
            >
              <Wand2 size={16} />
              {aiLoading === 'detect' ? <RefreshCw className="animate-spin" size={14} /> : <span className="hidden xl:inline">Auto-detect</span>}
            </button>
            <button
            ...
            >
              <ImageMinus size={16} />
              {aiLoading === 'remove-bg' ? <RefreshCw className="animate-spin" size={14} /> : <span className="hidden xl:inline">Remove BG</span>}
            </button>
            <label
